...
//          "#ffd363", "#ffb363", "#ff8363", "#ff7363", "#ff6364"];
 var heatmapColor = d3.scale.linear()
  ...
  .domain(d3.extent(data, function(d1) { return d1.Total;  }))
  .interpolate(d3.interpolateHcl);//colors);
...
svg.selectAll("path")
	...
	// console.log("Val:"+d.properties.Total);
	if (value) {
	return d3.rgb(heatmapColor(d.properties.Total));//heatmapColor(d.properties.Total);
	} else {
	return "rgb(1,1,1)";
	}
	})	
...
function typeManagement(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Management";});
	runstates(data);
}

function typeService(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Service";});
	runstates(data);
}

function typeSales(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Sales";});
	runstates(data);
}
  
function typeFarming(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Farming";});
	runstates(data);
}

function typeConstruction(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Construction";});
	runstates(data);
}
...
function typeProduction(data){
	console.log("over");
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Production";});
	runstates(data);
}

function typeMilitary(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Military";});
	runstates(data);
}

function typeStudents(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Students";});
	runstates(data);
}

function typeRetirees(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Retirees";});
	runstates(data);
}

function typeNo(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "No";});
	runstates(data);
}

function typeHomemakers(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Homemakers";});
	runstates(data);
}

function typeUnemployed(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Unemployed";});
	runstates(data);
}

function typeUnknown(data){
 	data = data.filter(function (d1)
    {	d3.select("svg").remove();	return d1.Profession == "Unknown";});
	runstates(data);
}
